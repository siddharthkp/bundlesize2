...
const markDuplicates = config => {
  ...
  // 5. sort them back to their original positions

  config.files.forEach(rule => {
    rule.specificity = calculateSecificity(rule.path)
  })

  config.files.forEach((rule, index) => (rule.originalIndex = index))
  config.files.sort((a, b) => b.specificity - a.specificity)
  ...
  config.files.sort((a, b) => a.originalIndex - b.originalIndex)
  config.files.forEach(rule => delete rule.originalIndex)
